...
- 工具 MUST 从 `META-INF/MANIFEST.MF` 的 `UCX-Version` 字段读取 MAJOR.MINOR。
- 若 `UCX-Version` 字段缺失、MAJOR 非数字或为负，工具 MUST 拒绝该归档并返回明确错误。
- 若文件 MAJOR **高于**工具支持的最高 MAJOR，工具 MUST 拒绝归档而非静默读取（向前不兼容原则）。
- 若文件 MAJOR 等于工具支持的 MAJOR 且 MINOR **高于**工具支持的最高 MINOR，工具 SHOULD 以"忽略未知字段/段"的宽松策略继续读取，并向用户发出警告（存在部分新功能不可用的可能）。
- 若 MAJOR 与 MINOR 均不高于工具支持版本，工具 MUST 无告警正常处理。

| 工具支持 | 文件版本 | 行为 |
|---------|---------|------|
| 1.1     | 1.0     | 正常处理，无警告 |
| 1.1     | 1.1     | 正常处理，无警告 |
| 1.1     | 1.2     | 宽松读取，警告"部分新功能不可用" |
| 1.1     | 2.0     | **拒绝**（返回 UnsupportedVersion 错误） |
| 1.1     | 2.1     | **拒绝**（返回 UnsupportedVersion 错误） |

> 实现参考：`ucx-parse` 的常量 `SUPPORTED_UCX_MAJOR` 与函数 `validate_ucx_version` 即为本节规则的权威实现。
